...
- Given a quantity is updated, then a historical count record is stored.
...
- Test historical count record creation.
...
## Story 10: Store Historical Inventory Counts

As the Director, I want historical inventory counts stored over time so that inventory movement is not lost when current quantities are updated.

### Acceptance Criteria

- Given an inventory entry has a current quantity, when the quantity changes, then the prior and new count information can be reviewed historically.
- Given multiple count updates exist for the same inventory entry, when history is requested, then the count records are returned in date order.
- Given a quantity update is rejected as invalid, then no historical count record is created for the rejected update.
- Given Commodity and non-Commodity entries exist for the same item, then their historical counts remain separate.

### TDD Notes

- Test historical count creation for valid updates.
- Test no history is created for failed updates.
- Test history retrieval ordering.
- Test Commodity and non-Commodity history separation.
- Use SQLite integration tests to prove the persisted history model.

## Story 11: Generate Inventory Trend Reports

As the Director, I want inventory trend reports so that I can understand how item, category, and Commodity inventory levels change over time.

### Acceptance Criteria

- Given historical count records exist, when a trend report is generated for an item, then the report shows counts over time.
- Given historical count records exist across categories, when a trend report is generated by category, then the report summarizes movement over time by category.
- Given Commodity and non-Commodity records exist, when trend reporting is filtered by Commodity status, then the report includes only the selected inventory type.
- Given no historical records exist for a requested report, then the report shows a clear empty-result message.

### TDD Notes

- Test item-level trend aggregation.
- Test category-level trend aggregation.
- Test Commodity-filtered trend aggregation.
- Test empty trend report behavior.
- Keep the first report format simple until staff validates what trend views are most useful.
